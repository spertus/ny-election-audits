...
audit_results <- function(true_counts, reported_counts, machine_types){
  ...
  for(i in 1:length(unique_machine_types)){
    ...
    initial_sample_size <- ceiling(.03 * length(index))
    initial_machines_sampled <- base::sample(index, size = initial_sample_size, replace = FALSE)
    ...
    discrepancies <- initial_handcount_samples - initial_reported_samples
    
    #total handcounts within each machine type
    handcount_samples_type[i,] <- colSums(initial_handcount_samples)
    reported_samples_type[i,] <- colSums(initial_reported_samples)
  ...
  positive_discrepancies_type <- colSums(positive_discrepancies)
  negative_discrepancies_type <- colSums(negative_discrepancies)
